...
def quat_diff_active(source_quat: np.ndarray, target_quat: np.ndarray) -> np.ndarray:
    source_xyzw = np.array([source_quat[1], source_quat[2], source_quat[3], source_quat[0]])
    target_xyzw = np.array([target_quat[1], target_quat[2], target_quat[3], target_quat[0]])
    
    r_source = R.from_quat(source_xyzw)
    r_target = R.from_quat(target_xyzw)
    
    r_diff = r_target * r_source.inv()
    
    diff_xyzw = r_diff.as_quat()
    return np.array([diff_xyzw[3], diff_xyzw[0], diff_xyzw[1], diff_xyzw[2]])
...
def opspace(
    ...
    if ori is None:
        ...
        quat_des = mat_to_quat(xmat)
    else:
        ori = np.asarray(ori)
        if ori.shape == (3, 3):
            quat_des = mat_to_quat(ori)
        else:
            quat_des = ori
    if joint is None:

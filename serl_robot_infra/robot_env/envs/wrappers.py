...
class MultiCameraAndEnvBinaryRewardClassifierWrapper(MultiCameraBinaryRewardClassifierWrapper):
    def __init__(self, env: Env, reward_classifier_func, target_hz = None):
        super().__init__(env, reward_classifier_func, target_hz)

    def step(self, action):
        start_time = time.time()
        obs, rew_env, done, truncated, info = self.env.step(action)
        rew_classifier = self.compute_reward(obs)
        rew = rew_env and rew_classifier
...
class Quat2RotvecWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        tcp_pose = observation["state"]["tcp_pose"]
        observation["state"]["tcp_pose"] = np.concatenate(
            (tcp_pose[:3], R.from_quat(tcp_pose[3:]).as_rotvec())
        )
        return observation
